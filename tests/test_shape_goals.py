...
def test_template_includes_shape_leans():
    ...
    assert "shape leans tanyao (2–8 only; no 1/9, winds, or dragons)" in result.summary
...
def test_template_glosses_multiple_goals():
    ...
    assert "yakuhai (dragon or seat/round wind)" in result.summary
    assert "dora (bonus tile)" in result.summary
    assert validate_explanation(turn, result) == []
...
def test_grounding_rejects_unlisted_yaku():
    ...
    bad = Explanation(
        summary="Mortal prefers 5-sou over 9-pin; keep pinfu.",
        focus="efficiency",
...
def test_grounding_allows_listed_yaku_and_dora():
    ...
    ok = Explanation(
        summary=(
            "Mortal prefers 5-sou; shape leans "
            "honitsu (one suit + winds/dragons OK) / "
            "yakuhai (dragon or seat/round wind) with dora (bonus tile) 5-man."
        ),
        focus="value",
        pinned_action="dahai 5s",
        contrasted_action="dahai 9p",
