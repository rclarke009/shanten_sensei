...
def test_anchored_llm_style_passes():
    turn = _turn(shape_goals=["tanyao"], dora_in_hand=["3s"])
    summary = (
        "Mortal prefers 3-pin over 5-man; you're 3-shanten (3 steps from ready) "
        "with about 51 acceptances (tiles that improve the hand); "
        "shape leans tanyao (2–8 only; no 1/9, winds, or dragons) with "
...
def test_wall_note_alt_live_acceptances():
    turn = _turn(
        diverge=True,
    ...
    note = wall_note(turn)
    assert note is not None
    assert "live acceptances" in note
    assert "~8 vs ~3" in note
...
def test_depletion_language_anchors_ukeire():
    ...
    summary = (
        "Mortal prefers 3-pin; several improving tiles are already out "
        "(only 1× 4-sou left)."
    )
    score = score_explanation_substance(turn, summary)
    assert score.thin is False
